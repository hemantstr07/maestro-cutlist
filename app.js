...
function addPartRow(data = {}) {
  ...
  tr.innerHTML = `
    ...
    <td style="text-align:center;"><input class="td-check" type="checkbox" data-field="rotate" ${data.rotate !== false ? 'checked' : ''} /></td>
 ...
 */
function guillotinePack({ boardW, boardH, kerf, edgeTrim, parts, opts }) {
  ...
  for (const item of items) {
    ...
    for (const board of boards) {
      ...
      if (bestFrIdx !== -1) {
        ...
        // Record placement
        // When grain is considered, visual grain = board grain (panels align with board)
...
function renderAllBoards() {
  ...
  for (const group of state.groups) {
    ...
    group.boards.forEach((board, bi) => {
      ...
      lbl.textContent = `Board ${bi + 1} of ${totalBoards}  ·  ${board.placements.length} piece${board.placements.length !== 1 ? 's' : ''}`;
...
function optimize() {
  ...
  for (const [thickness, groupParts] of thicknessMap) {
    const boards = guillotinePack({ boardW, boardH, kerf, edgeTrim, parts: groupParts, opts });
...
(function init() {
  const defaultParts = [
    { name: 'Side Panel',   width: 720,  height: 400, qty: 2, rotate: true  },
    { name: 'Top Panel',    width: 800,  height: 400, qty: 1, rotate: true  },
    { name: 'Bottom Panel', width: 800,  height: 400, qty: 1, rotate: true  },
    { name: 'Back Panel',   width: 800,  height: 700, qty: 1, rotate: false },
    { name: 'Shelf',        width: 760,  height: 350, qty: 2, rotate: true  },
    { name: 'Door',         width: 380,  height: 700, qty: 2, rotate: false },
  ];
